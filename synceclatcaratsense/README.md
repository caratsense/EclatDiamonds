...
- The watermark only advances after **every** upload chunk returns 2xx, so a
  ...
  Any partial run deliberately never advances it.
...
1. **Discover** (`1_discover.bat` / `python discover.py`) — read-only. The report
...
| Var | Meaning |
|---|---|
| `ECLAT_BASE_URL` | Eclat backend URL(s), comma-separated for several targets |
| `ECLAT_EMAIL` / `ECLAT_PASSWORD` | sync service-account login (secret) |
