...
def main():
    ...
    profile_store_entity_matrix(cur)
...
# ── Per-store x per-entity matrix ──────────────────────────────────────────────
# The profiler above answers "which column is the branch?"; this answers the next
# question "so how do the rows actually split across the shops?". Same resolved
# branch column per entity, one GROUP BY, three buckets: a known branch, no branch
# value at all (Missing Store), or a value the store catalog will not recognise
# (Unknown Mapping). Read-only COUNT/SELECT — never writes, never uploads.

# label, source table, branch-column candidates (None = not per-store), kind.
# Candidates mirror BRANCH_CANDIDATES / the extractors in sync_sjep.py; "BookNo"
# means resolve one hop through BookMaster (sales/orders/ledger carry no location).
MATRIX_ENTITIES = [
    ("stores",        "PartyMst",      None,                                               "stores"),
    ("products",      "StyleMst",      None,                                               "global"),
    ("stock",         "Inward",        ["LocationId", "BranchNo", "FirstLocationId"],       "direct"),
    ("customers",     "PartyMst",      ["BranchNo", "LocationId"],                          "customers"),
    ("sales",         "JewelTrans",    ["BranchNo", "LocationId", "BookNo"],                "auto"),
    ("payments",      "Journal",       ["BranchNo", "LocationId", "BookNo"],                "unconfirmed"),
    ("orders",        "Spm_MfgOrder",  ["BranchNo", "LocationId", "BookNo"],                "auto"),
    ("manufacturing", "SPM_BagMaster", ["CompanyId", "BranchNo", "BookNo"],                 "auto"),
    ("photos",        "StyleMst",      None,                                               "photos"),
]


def _cols(cur, table):
    """Lowercased->actual column map, or {} if the table is absent."""
    if not table_exists(cur, table):
        return {}
    try:
        cur.execute(f"SELECT TOP 0 * FROM [{table}]")
        return {str(d[0]).lower(): str(d[0]) for d in cur.description}
    except Exception:
        return {}


def _referenced_branches(cur):
    """The store universe the sync will actually create — party ids the DATA
    references as a branch. MIRRORS push_stores() in sync_sjep.py on purpose: the
    IsLocation/IsFactory flag is deliberately NOT the source of truth here (on the
    live install it returned suppliers and a test row 'abc'; the ids stock/sales
    point at are a different set). Returns (id_set, {id: firm_name})."""
    ref = set()
    for table, col in (("Inward", "BranchNo"), ("PartyMst", "BranchNo"),
        ...
        if col.lower() not in c:
            continue
        try:
            cur.execute(f"SELECT DISTINCT [{c[col.lower()]}] AS v FROM [{table}] "
                        f"WHERE [{c[col.lower()]}] IS NOT NULL")
            for r in rows(cur):
            ...
            pass
    names = {}
    pc = _cols(cur, "PartyMst")
    pk, nm = pc.get("partyno"), (pc.get("firmname") or pc.get("legalname"))
    if pk and nm:
        try:
            cur.execute(f"SELECT {pk} AS id, {nm} AS name FROM PartyMst")
            for r in rows(cur):
                names[str(r["id"]).strip()] = str(r.get("name") or "").strip()
        except Exception:
            pass
    return ref, names


def _book_branch(cur):
    """BookNo -> branch id via BookMaster (mirrors build_book_branch_map)."""
    c = _cols(cur, "BookMaster")
    pk = c.get("bookno")
    if not pk:
        return {}
    bc = next((c[x] for x in ("branchno", "locationid", "companyid", "branchid", "partyno")
               if x in c), None)
    if not bc:
        return {}
    m = {}
    try:
        cur.execute(f"SELECT {pk} AS b, {bc} AS br FROM BookMaster")
        for r in rows(cur):
            b, br = r.get("b"), r.get("br")
            if b is not None and br is not None and str(br).strip():
                m[str(b).strip()] = str(br).strip()
    except Exception:
        pass
    return m


def _bucket(pairs, ref, mapfn=None):
    """pairs: list of (raw_value, count). Split into a known-branch dict plus a
    Missing-Store count (NULL/blank) and an Unknown-Mapping count (a value that is
    not in the known branch set). mapfn resolves one hop (BookNo -> branch id)."""
    known, missing, unknown = {}, 0, 0
    for v, n in pairs:
        raw = None if v is None else str(v).strip()
        if not raw:
            missing += n
            continue
        resolved = mapfn(raw) if mapfn else raw
        if not resolved:                 # a book with no branch, unmappable
            unknown += n
        elif resolved in ref:
            known[resolved] = known.get(resolved, 0) + n
        else:
            unknown += n
    return known, missing, unknown


def _mrow(store, entity, count, missing="", unknown="", note=""):
    line = (f"  {str(store)[:36]:<36} {str(entity):<14} {str(count):>7} "
            f"{missing:^13} {unknown:^15}")
    if note:
        line += f"  {note}"
    out(line.rstrip())


def profile_store_entity_matrix(cur):
    out("\n" + "=" * 72)
    out("PER-STORE x PER-ENTITY MATRIX (read-only)")
    out("=" * 72)
    out("  How each entity's rows split across the branches the sync will create.")
    out("  Count = rows with that branch value. 'Missing Store' marks the bucket of")
    out("  rows carrying NO branch value (they land in the default store). 'Unknown")
    out("  Mapping' marks a branch value the store catalog will not recognise.")
    out("  Known-branch set mirrors push_stores(): ids the DATA references, NOT the")
    out("  IsLocation flag. Products and photos are company-global (storeId null).")
    out("")

    ref, names = _referenced_branches(cur)
    book = _book_branch(cur)
    if ref:
        shown = ", ".join(f"[{b}]{(' ' + names[b]) if names.get(b) else ''}"
                          for b in sorted(ref)[:30])
        out(f"  Known branches ({len(ref)}): {shown}"
            + (" ..." if len(ref) > 30 else ""))
    else:
        out("  ! No branch id is referenced by any record — every store-scoped row")
        out("    below will fall in the 'Missing Store' bucket.")
    out("")
    _mrow("Store", "Entity", "Count", "Missing", "Unknown")
    out("  " + "-" * 88)

    for label, table, cands, kind in MATRIX_ENTITIES:
        if kind == "stores":
            if ref:
                for b in sorted(ref):
                    _mrow(f"[{b}] {names.get(b, '')}".strip(), label, 1)
            else:
                _mrow("(none identified)", label, 0, note="no branch referenced by data")
            continue

        if kind == "global":
            n = count(cur, table)
            _mrow("GLOBAL", label, n if isinstance(n, int) else 0,
                  note="company-global (storeId null) — not per-store")
            continue

        if kind == "photos":
            cols = _cols(cur, table)
            img = cols.get("imagename")
            if not img:
                _mrow("GLOBAL", label, "?",
                      note="SOURCE UNCONFIRMED: no ImageName column on StyleMst")
            else:
                try:
                    cur.execute(f"SELECT COUNT(*) FROM [{table}] WHERE [{img}] IS NOT NULL "
                                f"AND LTRIM(RTRIM([{img}])) <> ''")
                    n = cur.fetchone()[0]
                except Exception:
                    n = "?"
                _mrow("GLOBAL", label, n,
                      note="catalogue-global; image FILES live under SJEP_IMAGE_ROOT on disk")
            continue

        # Table-backed, per-store entities (direct / customers / auto / unconfirmed).
        cols = _cols(cur, table)
        if not cols:
            _mrow("(table absent)", label, 0, note=f"{table} not found on this install")
            continue

        direct_col = None
        for cand in cands:
            if cand.lower() == "bookno":
                continue
            if cand.lower() in cols:
                direct_col = cols[cand.lower()]
                break
        where = ""
        if kind == "customers" and "iscustomer" in cols:
            where = f" WHERE [{cols['iscustomer']}] = 1"

        src_note = ""
        if kind == "unconfirmed":
            src_note = "SOURCE UNCONFIRMED: Journal vs VoucherEntry — confirm receipt source"

        if direct_col:
            grpcol, mapfn = direct_col, None
            resolved_desc = f"resolved via {direct_col}"
        elif "bookno" in cols and book:
            grpcol, mapfn = cols["bookno"], (lambda v: book.get(v))
            resolved_desc = "resolved via BookNo -> BookMaster"
        else:
            n = count(cur, table)
            _mrow("(unresolved)", label, n if isinstance(n, int) else 0,
                  note="; ".join(x for x in (src_note, "no branch column & no BookMaster path") if x))
            continue

        out(f"    -- {label}: {resolved_desc}" + (f"  [{src_note}]" if src_note else ""))
        try:
            cur.execute(f"SELECT [{grpcol}] AS v, COUNT(*) AS n FROM [{table}]{where} "
                        f"GROUP BY [{grpcol}]")
            pairs = [(r["v"], r["n"]) for r in rows(cur)]
        except Exception as e:
            _mrow("(query failed)", label, 0, note=str(e)[:50])
            continue

        known, missing, unknown = _bucket(pairs, ref, mapfn)
        for b in sorted(known):
            _mrow(f"[{b}] {names.get(b, '')}".strip(), label, known[b])
        if missing:
            _mrow("(no branch value)", label, missing, missing="X")
        if unknown:
            _mrow("(unknown branch id)", label, unknown, unknown="X")
        if not (known or missing or unknown):
            _mrow("(no rows)", label, 0)

    out("")
    out("  X in 'Missing Store'   = rows the branch column left blank -> default store.")
    out("  X in 'Unknown Mapping' = a branch value not in the store catalog above.")
    out("  Both should be near zero for a clean per-branch cutover; investigate if not.")


def _matrix_demo():
    """Offline self-check for the bucket logic (no DB needed)."""
    ref = {"10", "20"}
    known, missing, unknown = _bucket([("10", 5), ("20", 3), (None, 2), ("99", 1), ("", 4)], ref)
    assert known == {"10": 5, "20": 3}, known
    assert missing == 6 and unknown == 1, (missing, unknown)
    # book resolution: B1->10 (known), B2->99 (unknown branch), B9 unmapped
    book = {"B1": "10", "B2": "99"}
    known, missing, unknown = _bucket(
        [("B1", 7), ("B2", 2), ("B9", 1), (None, 3)], ref, lambda v: book.get(v))
    assert known == {"10": 7} and missing == 3 and unknown == 3, (known, missing, unknown)
    print("matrix demo OK")
