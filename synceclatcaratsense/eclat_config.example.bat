...
REM Branch legacyId to hand to a controlled --store run, e.g.
REM   python sync_sjep.py --sample --store %SYNC_DEFAULT_STORE_ID%
REM Read it off the discovery report's per-store matrix. Purely a convenience for
REM operators — it does NOT scope a normal (uncontrolled) sync.
set "SYNC_DEFAULT_STORE_ID="
...
REM  ↓ FILL IN — the two R2 API keys (secrets). Reuse the SAME keys already set on
REM    the Railway backend (R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY), or mint a new
REM    R2 API token with Object Read & Write on the eclat-media bucket. ↓
set "R2_ACCESS_KEY_ID="
set "R2_SECRET_ACCESS_KEY="
set "R2_BUCKET=eclat-media"
set "R2_PUBLIC_BASE_URL=https://pub-YOUR_BUCKET_ID.r2.dev"
