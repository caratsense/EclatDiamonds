...
export default function TimelinesPage() {
  ...
  // The selected order defaults to the first row. `active` is derived, so a
  // store switch or kind filter that drops the selected id simply falls back
  // to the first order without any effect-driven state reset.
  ...
  return (
    <>
      <SectionHeader
        ...
        purpose="Order booking and the real-time production engine for custom and stock orders. Internal operations view."
        primaryAction="Book Order"
        onPrimaryAction={() => setBookingOpen(true)}
      />
      ...
      <div className="mb-4 flex items-center gap-2 rounded-md border border-dashed bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
        ...
        Customer-facing read-only timeline is an open decision — showing the
        internal operations view only.
      </div>
      ...
      <Tabs defaultValue="orders">
        ...
        <TabsContent value="orders" className="space-y-4">
          {/* Kind filter — All / Custom / Stock. Store scope comes from the
              global store selector (X-Store-Id), so the list stays store-aware. */}
          ...
          {ordersLoading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-sm text-muted-foreground">
                No ongoing orders here yet — book one with Book Order.
              </CardContent>
            </Card>
          ) : (
            <>
