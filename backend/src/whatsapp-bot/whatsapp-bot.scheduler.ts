...
export class WhatsAppBotScheduler {
  ...
  async sweep(): Promise<void> {
    ...
    // This is a cross-tenant maintenance sweep: WhatsAppEvents arrive before the
    // sender's organisation is known (an unlinked number has none), so the sweep
    // is not per-org and the run ledger records it with a null organisation.
    // Exactly-once processing is enforced per-event inside processPending()
    // (the conditional `status: received -> processing` claim), not by this
    // wrapper — the wrapper just keeps replicas from all scanning at once.
